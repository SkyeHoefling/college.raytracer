...
RayColor ShaderTool::illuminate(Ray * ray, int depth){
	...
	if(!intersection) {
		...
		resultColor = collisionPoint.getShape()->getColor();
		if(depth < _maxDepth){
			if(collisionPoint.getShape()->getReflection() > 0){
				Ray * reflection = spawnReflectionRay(&collisionPoint, &ray->asVector3().normalized());
	...
	return resultColor.getNormalized();
}
...
Ray* ShaderTool::spawnReflectionRay(PointCollision* collisionPoint, Vector3* currentRay){
	...
	// a = ----------- N
	//		  |N|^2
	double a = currentRay->normalized().dot(collisionPoint->getSurfaceNormal());
	a /= collisionPoint->getSurfaceNormal().length() * collisionPoint->getSurfaceNormal().length();
	Vector3 reflectionNormal = collisionPoint->getSurfaceNormal() * a * 2;
	reflectionNormal = *currentRay - reflectionNormal;
	reflectionNormal.normalize();
	Ray* reflection = new Ray(collisionPoint->getPosition());
	reflection->setTarget(reflectionNormal * 1000);
	return reflection;
